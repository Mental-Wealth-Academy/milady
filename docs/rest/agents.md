...
All agent endpoints require the agent runtime to be initialized. The API server runs on port **31337** in development (`MILADY_API_PORT`) and **2138** in production (`milady start`). All paths are prefixed with `/api/`. When `MILADY_API_TOKEN` is set, include it as a `Bearer` token in the `Authorization` header.
